    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          {(['1M', '3M', '6M', '1Y'] as const).map((p) => (
            <button key={p} onClick={() => setPeriod(p)}
              className={`px-3 py-1 rounded text-xs font-medium ${period === p ? 'bg-[#d4e157]/20 text-[#d4e157]' : 'bg-slate-800/50 text-slate-400'}`}>
              {p}
            </button>
          ))}
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          <div key={m.label} className="chart-glass p-4">
        ...
        <div className="chart-glass p-6">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={revenueData}>
              ...
              <YAxis stroke="#777" fontSize={12} tickFormatter={(v) => `R${(v / 1000).toFixed(0)}K`} />
              <Tooltip contentStyle={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '8px' }}
                formatter={(value: number) => [`R${(value / 1000).toFixed(0)}K`]} />
              <Bar dataKey="revenue" fill="#d4e157" radius={[4, 4, 0, 0]} name="Revenue" />
        ...
        <div className="chart-glass p-6">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={volumeData}>
              ...
              <Tooltip contentStyle={{ background: '#1e293b', border: '1px solid #334155', borderRadius: '8px' }} />
              <Area type="monotone" dataKey="solar" stackId="1" stroke="#d4e157" fill="#d4e157" fillOpacity={0.3} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="chart-glass p-6 col-span-2">
          <h3 className="text-sm font-semibold mb-4">30-Day Price History (R/MWh)</h3>
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={priceHistory}>
              <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
              <XAxis dataKey="day" stroke="#777" fontSize={11} />
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {[
                ...
                <tr key={a.asset} className="border-t border-slate-800">
                  <td className="py-2 font-medium">{a.asset}</td>
                  <td>{a.market}</td>
                  <td className="text-right">{a.volume}</td>
                  <td className="text-right text-[#d4e157]">{a.revenue}</td>
