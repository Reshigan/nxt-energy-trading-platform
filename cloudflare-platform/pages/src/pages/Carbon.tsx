...
export default function Carbon() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <h1 className="text-2xl font-bold gradient-text">Carbon Credits</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          { label: 'Total Value', value: `R${totalValue.toLocaleString()}`, color: 'text-[#d4e157]' },
          ...
          <div className="chart-glass p-6">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={inventoryChart}>
                ...
                <Bar dataKey="value" fill="#d4e157" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-end">
            {(activeRole === 'trader' || activeRole === 'carbon_fund' || activeRole === 'admin') && (
              <button onClick={() => setShowWriteOptionModal(true)} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-[#d4e157] to-[#b8c43a] text-slate-900 rounded-lg text-sm font-medium">
                    ...
                    {options.map((o) => (
                      <tr key={o.id as string} className="border-t border-slate-800">
                        ...
                        <td className="text-right">
                          {o.status === 'active' && (
                            <button onClick={() => { setSelectedOption(o); setShowExerciseModal(true); }} className="text-xs text-[#d4e157] hover:text-[#b8c43a]">Exercise</button>
                          )}
                        </td>
                      </tr>
                    ))}
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={m.label} className="chart-glass p-4 text-center">
                <div className="text-xl font-bold text-[#d4e157]">{String(m.value)}</div>
                <div className="text-xs text-slate-400 mt-1">{m.label}</div>
              </div>
            ))}
          </div>
      ...
      {tab === 'registry' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[
            ...
            <div key={reg.name} className="chart-glass p-6">
              <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-3xl font-bold text-[#d4e157] mb-2">{reg.credits} credits</div>
              <button onClick={() => handleSync(reg.name.toLowerCase().replace(/[^a-z]/g, '_'))}
                className="flex items-center gap-2 text-sm text-slate-400 hover:text-white mt-4">
      ...
      <Modal isOpen={showRetireModal} onClose={() => setShowRetireModal(false)} title="Retire Carbon Credits">
        <form onSubmit={handleRetire} className="space-y-4">
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1">Quantity to Retire *</label>
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1">Beneficiary</label>
            <input className={inputClass} value={retireForm.beneficiary} onChange={(e) => setRetireForm({ ...retireForm, beneficiary: e.target.value })} />
          </div>
          <div>
            <label className="block text-xs text-slate-400 mb-1">Retirement Statement</label>
      ...
      <Modal isOpen={showTransferModal} onClose={() => setShowTransferModal(false)} title="Transfer Carbon Credits">
        <form onSubmit={handleTransfer} className="space-y-4">
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1">Recipient Account ID *</label>
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1">Recipient Organisation</label>
      ...
      <Modal isOpen={showWriteOptionModal} onClose={() => setShowWriteOptionModal(false)} title="Write Carbon Option" size="lg">
        <form onSubmit={handleWriteOption} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs text-slate-400 mb-1">Strike Price (R) *</label>
            ...
            <div>
              <label className="block text-xs text-slate-400 mb-1">Premium (R) *</label>
            ...
            <div>
              <label className="block text-xs text-slate-400 mb-1">Settlement</label>
            ...
            <div>
              <label className="block text-xs text-slate-400 mb-1">Underlying Credit ID</label>
      ...
      <Modal isOpen={showExerciseModal} onClose={() => setShowExerciseModal(false)} title="Exercise Option">
        {selectedOption && (
          <div className="space-y-4">
            ...
            <button onClick={handleExercise} disabled={loading} className="w-full py-2.5 bg-gradient-to-r from-[#d4e157] to-[#b8c43a] text-slate-900 rounded-lg font-medium disabled:opacity-50">
