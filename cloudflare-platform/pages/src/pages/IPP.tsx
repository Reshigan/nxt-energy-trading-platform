  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold gradient-text">IPP Projects</h1>
        {(activeRole === 'ipp_developer' || activeRole === 'admin') && (
          <button onClick={() => setShowCreateModal(true)} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-[#d4e157] to-[#b8c43a] text-slate-900 rounded-lg text-sm font-medium">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Projects', value: projects.length, color: 'text-[#d4e157]' },
          ...
          { label: 'Operational', value: operational, color: 'text-cyan-400' },
        ].map((m) => (
      ...
      {selectedProject ? (
        <div className="chart-glass p-6 space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <button onClick={() => setSelectedProject(null)} className="text-xs text-slate-400 hover:text-white mb-2">&larr; Back</button>
              <h2 className="text-lg font-bold">{selectedProject.name as string}</h2>
              <div className="flex gap-2 mt-1">
                <StatusBadge status={selectedProject.phase as string} />
                <span className="text-xs text-slate-400">{selectedProject.capacity_mw as number} MW {selectedProject.technology as string}</span>
              </div>
            </div>
            <div className="text-right text-sm">
              <div className="text-slate-400">Progress</div>
              <div className="text-[#d4e157] font-bold text-xl">{selectedProject.progress_pct as number || 0}%</div>
            </div>
          </div>
          ...
          <div className="flex gap-1">
            {phases.map((p, i) => {
              ...
              return (
                <div key={p} className={`flex-1 text-center py-2 rounded text-xs font-medium ${isCurrent ? 'bg-[#d4e157]/20 text-[#d4e157]' : isPast ? 'bg-emerald-500/10 text-emerald-400' : 'bg-slate-800/50 text-slate-500'}`}>
                  {p.replace('_', ' ')}
                </div>
              );
            })}
          ...
          <div className="flex gap-2">
            {(['milestones', 'cps', 'disbursements', 'docs'] as const).map((t) => (
              <button key={t} onClick={() => setDetailTab(t)}
                className={`px-4 py-2 rounded-lg text-sm font-medium capitalize transition-colors ${detailTab === t ? 'bg-[#d4e157]/20 text-[#d4e157] border border-[#d4e157]/30' : 'bg-slate-800/50 text-slate-400'}`}>
          ...
          {detailTab === 'milestones' && (
            <div className="space-y-3">
              {MILESTONES.map((ms, i) => (
                <div key={ms} className="flex items-center gap-4 p-4 rounded-lg bg-slate-800/50">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-slate-400">{i < 3 ? 'Completed' : 'Pending'}</div>
                  </div>
                  {i >= 3 && (activeRole === 'ipp_developer' || activeRole === 'admin') && (
                    <button onClick={() => handleCompleteMilestone(`ms-${i}`)} className="px-3 py-1 text-xs bg-[#d4e157]/20 text-[#d4e157] rounded-lg">
                      Complete
                    </button>
                  )}
                </div>
          ...
          {detailTab === 'cps' && (
            <div className="space-y-3">
              {CP_LIST.map((cp, i) => (
                <div key={cp} className="flex items-center gap-4 p-4 rounded-lg bg-slate-800/50">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-slate-400">{i < 4 ? 'Satisfied' : i < 7 ? 'In Progress' : 'Outstanding'}</div>
                  </div>
          ...
          {detailTab === 'disbursements' && (
            <div className="space-y-3">
              {[
                ...
                <div key={d.milestone} className="flex items-center justify-between p-4 rounded-lg bg-slate-800/50">
                  <div>
                    <div className="font-medium text-sm">{d.milestone}</div>
                    <div className="text-lg font-bold text-[#d4e157]">{d.amount}</div>
                  </div>
          ...
          {detailTab === 'docs' && (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {['EIA ROD', 'NERSA Licence', 'Grid Agreement', 'PPA Draft', 'PPA Signed', 'EPC Contract', 'Insurance Pack', 'Land Lease', 'BBBEE Cert', 'Legal Opinion', 'IE Certificate', 'COD Certificate'].map((doc) => (
                <div key={doc} className="p-4 rounded-lg bg-slate-800/50 hover:bg-slate-800 transition-colors cursor-pointer text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {projects.length === 0 ? (
            <div className="col-span-2 chart-glass p-12 text-center">
              <FiZap className="w-12 h-12 text-slate-600 mx-auto mb-4" />
              <p className="text-slate-400">No IPP projects yet</p>
            </div>
          ) : projects.map((p) => (
            <div key={p.id as string} className="chart-glass p-5 cursor-pointer hover:border-[#d4e157]/30 transition-colors" onClick={() => loadProjectDetail(p)}>
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold">{p.name as string}</h3>
                <StatusBadge status={p.phase as string} />
              </div>
              <div className="grid grid-cols-3 gap-2 mb-3">
                <div><div className="text-xs text-slate-400">Capacity</div><div className="font-medium">{p.capacity_mw as number} MW</div></div>
                <div><div className="text-xs text-slate-400">Technology</div><div className="font-medium capitalize">{p.technology as string}</div></div>
                <div><div className="text-xs text-slate-400">Province</div><div className="font-medium">{p.province as string || '-'}</div></div>
              </div>
              <div className="w-full bg-slate-800 rounded-full h-1.5">
                <div className="bg-gradient-to-r from-[#d4e157] to-[#b8c43a] h-1.5 rounded-full" style={{ width: `${p.progress_pct as number || 0}%` }} />
              </div>
              <div className="text-xs text-slate-400 mt-1">{p.progress_pct as number || 0}% complete • COD: {p.cod_target as string || 'TBD'}</div>
      ...
      <Modal isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} title="New IPP Project">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs text-slate-400 mb-1">COD Target</label>
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1">Grid Connection Point</label>
          ...
          <button type="submit" disabled={loading} className="w-full py-2.5 bg-gradient-to-r from-[#d4e157] to-[#b8c43a] text-slate-900 rounded-lg font-medium disabled:opacity-50">
      ...
      <Modal isOpen={showUploadCPModal} onClose={() => setShowUploadCPModal(false)} title="Upload CP Document">
        <div className="space-y-4">
          ...
          <button className="w-full py-2.5 bg-gradient-to-r from-[#d4e157] to-[#b8c43a] text-slate-900 rounded-lg font-medium">
            Submit Evidence
          </button>
        </div>
      </Modal>
