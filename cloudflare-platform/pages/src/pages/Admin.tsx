...
export default function Admin() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <h1 className={`text-2xl font-bold ${tc.textPrimary}`}>Admin Panel</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={stat.label} className={`${tc.cardBg} p-4`}>
      ...
      <div className="flex gap-2">
        {tabs.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === t.id ? 'bg-blue-500/15 text-blue-400 border border-blue-500/30' : 'bg-slate-100 dark:bg-white/[0.04] text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-white/[0.08]'}`}>
          ...
          {participants.length === 0 ? <p className="text-sm text-slate-400">No participants</p> : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {participants.map((p) => (
                    <tr key={p.id as string} className="border-t border-slate-200 dark:border-white/[0.06]">
                ...
                <tbody>
                  {statutory.map((ch) => (
                    <tr key={ch.id as string} className="border-t border-slate-200 dark:border-white/[0.06]">
                      ...
                      <td className="text-xs text-slate-500 dark:text-slate-400">{ch.checked_at as string || '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className={`${tc.cardBg} p-6`}>
          {reports.length === 0 ? <p className="text-sm text-slate-400">No reports</p> : (
            <div className="space-y-3">
              {reports.map((r, i) => (
                <div key={i} className={`flex items-center justify-between p-4 rounded-lg ${tc.isDark ? "bg-white/[0.04]" : "bg-slate-50"}`}>
                  <div>
                    <div className="font-medium capitalize">{(r.type as string || '').replace(/_/g, ' ')}</div>
                    <div className="text-xs text-slate-500 dark:text-slate-400">Period: {r.period as string}</div>
                  </div>
                  <div className="text-right">
                    <StatusBadge status={r.status as string} />
                    <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">Due: {r.due_date as string}</div>
                  </div>
                </div>
              ))}
            </div>
