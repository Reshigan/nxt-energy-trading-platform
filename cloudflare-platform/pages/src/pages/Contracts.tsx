  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold gradient-text">Digital Contracts</h1>
      ...
      <div className="flex gap-2">
        {tabs.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === t.id ? 'bg-[#d4e157]/20 text-[#d4e157] border border-[#d4e157]/30' : 'bg-slate-800/50 text-slate-400 hover:bg-slate-800'}`}>
            {t.label}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-4">
          {selectedDoc ? (
            <div className="chart-glass p-6 space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <button onClick={() => setSelectedDoc(null)} className="text-xs text-slate-400 hover:text-white mb-2">&larr; Back to list</button>
                  <h2 className="text-lg font-bold">{selectedDoc.title as string}</h2>
                  <div className="flex gap-2 mt-1">
                    <StatusBadge status={selectedDoc.phase as string} />
                    <span className="text-xs text-slate-400">v{selectedDoc.version as string}</span>
                  </div>
                </div>
                <div className="flex gap-2">
              ...
              <div className="overflow-x-auto">
                <div className="flex gap-1 min-w-max">
                  {PHASES.map((p, i) => {
                    ...
                    return (
                      <div key={p} className={`flex items-center gap-1 px-3 py-1.5 rounded text-xs font-medium ${isCurrent ? 'bg-[#d4e157]/20 text-[#d4e157] border border-[#d4e157]/30' : isPast ? 'bg-emerald-500/10 text-emerald-400' : 'bg-slate-800/50 text-slate-500'}`}>
              ...
              <div>
                ...
                {signatures.length === 0 ? <p className="text-xs text-slate-400">No signatories</p> : (
                  <div className="space-y-2">
                    {signatures.map((s) => (
                      <div key={s.id as string} className="flex items-center justify-between p-3 rounded-lg bg-slate-800/50">
                        <div>
                          <div className="text-sm font-medium">{s.signatory_name as string}</div>
                          <div className="text-xs text-slate-400">{s.signatory_designation as string}</div>
                        </div>
                        {s.signed ? (
              ...
              <div>
                ...
                {versions.length === 0 ? <p className="text-xs text-slate-400">No versions</p> : (
                  <div className="space-y-2">
                    {versions.map((v) => (
                      <div key={v.id as string} className="flex items-center justify-between p-3 rounded-lg bg-slate-800/50">
                        <div className="text-sm">v{v.version as string} — {v.title as string}</div>
                        <div className="text-xs text-slate-400">{v.created_at as string}</div>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {documents.length === 0 ? <p className="text-sm text-slate-400">No documents found</p> : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {documents.map((d) => (
                        <tr key={d.id as string} className="border-t border-slate-800 cursor-pointer hover:bg-slate-800/30" onClick={() => loadDocDetail(d)}>
                          ...
                          <td className="text-xs text-slate-400">{(d.created_at as string)?.slice(0, 10)}</td>
                          <td className="text-right"><button className="text-xs text-[#d4e157]">View</button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="flex items-center gap-3 p-3 rounded-lg bg-slate-800/50">
                ...
                <FiFileText className="w-4 h-4 text-[#d4e157] flex-shrink-0" />
                <span className="px-2 py-1 rounded text-xs bg-slate-700 font-mono">{rule.to}</span>
                <span className="text-xs text-slate-400 flex-1">{rule.prereq}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {tab === 'templates' && (
        <div className="chart-glass p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {DOC_TYPES.map((t) => (
              <div key={t} className="p-4 rounded-lg bg-slate-800/50 hover:bg-slate-800 transition-colors cursor-pointer">
                <FiFileText className="w-8 h-8 text-[#d4e157] mb-2" />
                <div className="font-medium capitalize">{t.replace('_', ' ')}</div>
                <div className="text-xs text-slate-400 mt-1">Standard template</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <Modal isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} title="New Document" size="lg">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs text-slate-400 mb-1">Counterparty ID</label>
            ...
            <div>
              <label className="block text-xs text-slate-400 mb-1">Pricing Mechanism</label>
            ...
            <div>
              <label className="block text-xs text-slate-400 mb-1">Tenor</label>
              <input className={inputClass} value={createForm.tenor} onChange={(e) => setCreateForm({ ...createForm, tenor: e.target.value })} />
            </div>
          </div>
          <div>
            <label className="block text-xs text-slate-400 mb-1">Delivery Point</label>
      ...
      <Modal isOpen={showAmendModal} onClose={() => setShowAmendModal(false)} title="Create Amendment">
        <form onSubmit={handleAmend} className="space-y-4">
          <div>
            <label className="block text-xs text-slate-400 mb-1">Reason for Amendment *</label>
      ...
      <Modal isOpen={showPhaseModal} onClose={() => setShowPhaseModal(false)} title="Advance Phase">
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1">Target Phase *</label>
