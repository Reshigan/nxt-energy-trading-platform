...
popia.get('/export', authMiddleware(), async (c) => {
  ...
  const participant = await c.env.DB.prepare(
    'SELECT id, company_name, registration_number, tax_number, vat_number, role, contact_person, email, phone, physical_address, sa_id_number, bbbee_level, nersa_licence, fsca_licence, kyc_status, consent_given, consent_given_at, created_at, updated_at FROM participants WHERE id = ?'
  ).bind(user.sub).first();

  const contracts = await c.env.DB.prepare(
    'SELECT id, title, document_type, phase, version, created_at FROM contract_documents WHERE creator_id = ? OR counterparty_id = ?'
  ).bind(user.sub, user.sub).all();

  const trades = await c.env.DB.prepare(
    'SELECT id, market, volume, price_cents, total_cents, status, created_at FROM trades WHERE buyer_id = ? OR seller_id = ?'
  ).bind(user.sub, user.sub).all();

  const orders = await c.env.DB.prepare(
    'SELECT id, direction, market, volume, price_cents, order_type, status, created_at FROM orders WHERE participant_id = ?'
  ).bind(user.sub).all();

  const carbonCredits = await c.env.DB.prepare(
    'SELECT id, serial_number, project_name, registry, vintage, quantity, status, created_at FROM carbon_credits WHERE owner_id = ?'
  ).bind(user.sub).all();

  const notifications = await c.env.DB.prepare(
    'SELECT id, title, body, type, read, created_at FROM notifications WHERE participant_id = ?'
  ).bind(user.sub).all();

  const auditLogs = await c.env.DB.prepare(
    'SELECT id, action, entity_type, entity_id, details, ip_address, created_at FROM audit_log WHERE actor_id = ?'
  ).bind(user.sub).all();

  const kycDocs = await c.env.DB.prepare(
    'SELECT id, document_type, file_name, verified, created_at FROM kyc_documents WHERE participant_id = ?'
  ).bind(user.sub).all();
  ...
  return c.json({
    ...
    data: {
      ...
      carbon_credits: carbonCredits.results,
      notifications: notifications.results,
      audit_log: auditLogs.results,
      kyc_documents: kycDocs.results,
    },
  });
});
