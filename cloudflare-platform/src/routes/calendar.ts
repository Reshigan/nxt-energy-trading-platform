...
type CalendarEvent = { id: string; title: string; date: string; type: string; entity_type?: string; entity_id?: string; severity?: string };

async function fetchEvents(db: D1Database, pid: string, from: string, to: string): Promise<CalendarEvent[]> {
...
calendar.get('/', async (c) => {
  try {
    ...
    const events = await fetchEvents(c.env.DB, user.sub, from, to);
...
calendar.get('/today', async (c) => {
  try {
    ...
    const events = await fetchEvents(c.env.DB, user.sub, today, today);
    return c.json({ success: true, data: events });
  } catch (err) {
    captureException(c, err);
    return c.json({ success: true, data: [] });
...
calendar.get('/week', async (c) => {
  try {
    ...
    const events = await fetchEvents(c.env.DB, user.sub, today.toISOString().substring(0, 10), weekEnd.toISOString().substring(0, 10));
    return c.json({ success: true, data: events });
  } catch (err) {
    captureException(c, err);
    return c.json({ success: true, data: [] });
