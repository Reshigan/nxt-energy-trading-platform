    ...
    template: (data: { name: string; company: string }) => `
      <div style="max-width:600px;margin:0 auto;font-family:Inter,sans-serif">
        ...
        <div style="padding:32px;background:#fff">
          <h2 style="color:#0f172a">Welcome, ${data.name}!</h2>
          <p style="color:#475569">Your account for <strong>${data.company}</strong> has been created on the NXT Energy Trading Platform.</p>
    ...
    template: (data: { otp: string }) => `
      <div style="max-width:600px;margin:0 auto;font-family:Inter,sans-serif">
        ...
        <div style="padding:32px;background:#fff;text-align:center">
          ...
          <div style="font-size:36px;font-weight:700;letter-spacing:8px;color:#0f172a;padding:16px;background:#f1f5f9;border-radius:12px;display:inline-block;margin:16px 0">${data.otp}</div>
      ...
      <div style="max-width:600px;margin:0 auto;font-family:Inter,sans-serif">
        ...
        <div style="padding:32px;background:#fff">
          ...
          <table style="width:100%;border-collapse:collapse;margin:16px 0">
            <tr><td style="padding:8px;border-bottom:1px solid #e2e8f0;color:#64748b">Type</td><td style="padding:8px;border-bottom:1px solid #e2e8f0;font-weight:600">${data.trade_type}</td></tr>
            <tr><td style="padding:8px;border-bottom:1px solid #e2e8f0;color:#64748b">Volume</td><td style="padding:8px;border-bottom:1px solid #e2e8f0;font-weight:600">${data.volume}</td></tr>
            <tr><td style="padding:8px;border-bottom:1px solid #e2e8f0;color:#64748b">Price</td><td style="padding:8px;border-bottom:1px solid #e2e8f0;font-weight:600">${data.price}</td></tr>
            <tr><td style="padding:8px;border-bottom:1px solid #e2e8f0;color:#64748b">Market</td><td style="padding:8px;border-bottom:1px solid #e2e8f0;font-weight:600">${data.market}</td></tr>
            <tr><td style="padding:8px;color:#64748b">Settlement Date</td><td style="padding:8px;font-weight:600">${data.settlement_date}</td></tr>
          </table>
    ...
    template: (data: { invoice_number: string; total: string; due_date: string; seller: string; buyer: string }) => `
      <div style="max-width:600px;margin:0 auto;font-family:Inter,sans-serif">
        ...
        <div style="padding:32px;background:#fff">
          ...
          <p style="color:#475569">Invoice <strong>${data.invoice_number}</strong> has been generated.</p>
          <table style="width:100%;border-collapse:collapse;margin:16px 0">
            <tr><td style="padding:8px;border-bottom:1px solid #e2e8f0;color:#64748b">From</td><td style="padding:8px;border-bottom:1px solid #e2e8f0;font-weight:600">${data.seller}</td></tr>
            <tr><td style="padding:8px;border-bottom:1px solid #e2e8f0;color:#64748b">To</td><td style="padding:8px;border-bottom:1px solid #e2e8f0;font-weight:600">${data.buyer}</td></tr>
            <tr><td style="padding:8px;border-bottom:1px solid #e2e8f0;color:#64748b">Total</td><td style="padding:8px;border-bottom:1px solid #e2e8f0;font-weight:600;color:#059669">${data.total}</td></tr>
            <tr><td style="padding:8px;color:#64748b">Due Date</td><td style="padding:8px;font-weight:600">${data.due_date}</td></tr>
          </table>
    ...
    template: (data: { document_title: string; requester: string }) => `
      <div style="max-width:600px;margin:0 auto;font-family:Inter,sans-serif">
        ...
        <div style="padding:32px;background:#fff">
          ...
          <p style="color:#475569"><strong>${data.requester}</strong> has requested your signature on:</p>
          <p style="font-size:18px;font-weight:600;color:#0f172a">${data.document_title}</p>
    ...
    template: (data: { passed: number; total: number; checks: Array<{name: string; status: string}> }) => `
      <div style="max-width:600px;margin:0 auto;font-family:Inter,sans-serif">
        ...
        <div style="padding:32px;background:#fff">
          ...
          <p style="color:#475569"><strong>${data.passed}</strong> of <strong>${data.total}</strong> verification checks have passed.</p>
          <table style="width:100%;border-collapse:collapse;margin:16px 0">
            ${data.checks.map(c => `<tr><td style="padding:8px;border-bottom:1px solid #e2e8f0">${c.name}</td><td style="padding:8px;border-bottom:1px solid #e2e8f0;text-align:right;font-weight:600;color:${c.status === 'pass' ? '#059669' : c.status === 'fail' ? '#dc2626' : '#d97706'}">${c.status.toUpperCase()}</td></tr>`).join('')}
          </table>
    ...
    template: (data: { cp_description: string; days: number; project_name: string }) => `
      <div style="max-width:600px;margin:0 auto;font-family:Inter,sans-serif">
        ...
        <div style="padding:32px;background:#fff">
          ...
          <p style="color:#475569">A condition precedent for <strong>${data.project_name}</strong> is due soon:</p>
          <div style="background:#fef3c7;border-left:4px solid #f59e0b;padding:16px;border-radius:0 8px 8px 0;margin:16px 0">
            <p style="font-weight:600;color:#92400e;margin:0">${data.cp_description}</p>
            <p style="color:#92400e;margin:4px 0 0;font-size:14px">Due in ${data.days} days</p>
          </div>
      ...
      <div style="max-width:600px;margin:0 auto;font-family:Inter,sans-serif">
        ...
        <div style="padding:32px;background:#fff">
          ...
          <div style="background:#fef2f2;border-left:4px solid #ef4444;padding:16px;border-radius:0 8px 8px 0;margin:16px 0">
            <p style="font-weight:600;color:#991b1b;margin:0">Dispute ${data.dispute_id}</p>
            <p style="color:#991b1b;margin:4px 0 0;font-size:14px">${data.reason}</p>
          </div>
          <p style="color:#475569">Response deadline: <strong>${data.deadline}</strong></p>
  ...
  monthly_summary: {
    ...
    template: (data: { month: string; trading_volume: string; pnl: string; carbon_position: string; compliance_status: string }) => `
      <div style="max-width:600px;margin:0 auto;font-family:Inter,sans-serif">
        ...
        <div style="padding:32px;background:#fff">
          <h2 style="color:#0f172a">Monthly Summary — ${data.month}</h2>
          <table style="width:100%;border-collapse:collapse;margin:16px 0">
            <tr><td style="padding:12px;border-bottom:1px solid #e2e8f0;color:#64748b">Trading Volume</td><td style="padding:12px;border-bottom:1px solid #e2e8f0;font-weight:600;text-align:right">${data.trading_volume}</td></tr>
            <tr><td style="padding:12px;border-bottom:1px solid #e2e8f0;color:#64748b">P&L</td><td style="padding:12px;border-bottom:1px solid #e2e8f0;font-weight:600;text-align:right;color:#059669">${data.pnl}</td></tr>
            <tr><td style="padding:12px;border-bottom:1px solid #e2e8f0;color:#64748b">Carbon Position</td><td style="padding:12px;border-bottom:1px solid #e2e8f0;font-weight:600;text-align:right">${data.carbon_position}</td></tr>
            <tr><td style="padding:12px;color:#64748b">Compliance Status</td><td style="padding:12px;font-weight:600;text-align:right;color:#059669">${data.compliance_status}</td></tr>
          </table>
