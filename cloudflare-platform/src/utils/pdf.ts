...
<head>
  <meta charset="UTF-8">
  <title>${doc.title} — NXT Energy</title>
...
<body>
  <div class="cover">
    ...
    <h1>${doc.title}</h1>
    <p style="font-size: 12pt; color: #777;">Document Type: ${doc.document_type} | Version: ${doc.version}</p>
    <p style="font-size: 12pt; color: #777;">Phase: ${doc.phase} | Date: ${new Date().toLocaleDateString('en-ZA')}</p>
  </div>

  <h2>Document Details</h2>
  <table class="meta-table">
    <tr><td class="label">Document ID</td><td>${doc.id}</td></tr>
    <tr><td class="label">Creator</td><td>${doc.creator_id}</td></tr>
    <tr><td class="label">Counterparty</td><td>${doc.counterparty_id}</td></tr>
    <tr><td class="label">SHA-256 Hash</td><td class="hash">${doc.document_hash || 'Pending'}</td></tr>
  </table>
  ...
  <div class="sig-block">
    ${signatures.map(s => `
      <div class="sig-item">
        <strong>${s.signatory_name}</strong> — ${s.signatory_designation}<br>
        ${s.signed ? `<span class="badge-pass">Signed: ${s.signed_at}</span> | IP: ${s.ip_address}` : '<span class="badge-pending">Pending</span>'}
      </div>
    `).join('')}
  </div>

  <h2>Statutory Compliance</h2>
  ${checks.map(ch => `<p>${ch.regulation}: <span class="${ch.status === 'pass' ? 'badge-pass' : ch.status === 'fail' ? 'badge-fail' : 'badge-pending'}">${ch.status.toUpperCase()}</span></p>`).join('')}
...
export function generateInvoicePDF(invoice: InvoiceData, seller: ParticipantInfo, buyer: ParticipantInfo): string {
  ...
  const lineItemsHtml = (invoice.line_items || []).map(item => `
    <tr>
      <td style="padding: 8px; border: 1px solid #e8e4dc;">${item.description}</td>
...
<head>
  <meta charset="UTF-8">
  <title>Invoice ${invoice.invoice_number} — NXT Energy</title>
  <style>${COMMON_STYLES}
...
<body>
  ...
  <table class="meta-table" style="width: 400px;">
    ...
    <tr><td class="label">Reference</td><td>${invoice.invoice_number}</td></tr>
  </table>
...
<body>
  <div class="certificate">
    ...
    <table class="meta-table" style="width: 500px; margin: 20px auto; text-align: left;">
      ...
      <tr><td class="label">Serial Number</td><td>${credit.serial_number || 'N/A'}</td></tr>
      <tr><td class="label">Registry</td><td>${credit.registry || credit.standard || 'NXT'}</td></tr>
      <tr><td class="label">Vintage</td><td>${credit.vintage || 'N/A'}</td></tr>
      <tr><td class="label">Project</td><td>${credit.project_name || 'N/A'}</td></tr>
      <tr><td class="label">Beneficiary</td><td>${retirement.beneficiary}</td></tr>
      <tr><td class="label">Purpose</td><td>${retirement.purpose}</td></tr>
